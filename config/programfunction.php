...
{
	$stmt = $conn->prepare("INSERT INTO mmt_program(`tv_title`,`tv_description`,`status`,`image_path`,`created_date`) VALUES (:tv_title, :tv_description,:status, :image_path, :created_date)");
...
function getAllActiveProgram($conn)
{
	$stmt= $conn->prepare("SELECT * FROM mmt_program WHERE status='active' ORDER BY id ASC LIMIT 4  ");
	$stmt->execute();
...
function updateProgram($conn,$data)
{
	...
	{
	$stmt = $conn->prepare("UPDATE mmt_program SET tv_title=:tv_title, tv_description=:tv_description,status=:status, image_path=:image_path, created_date=:created_date WHERE id=:id");
	$stmt->bindParam(':tv_title',$data['tv_title']);
	$stmt->bindParam(':tv_description',$data['tv_description']);
	$stmt->bindParam(':status',$data['status']);
	$stmt->bindParam(':image_path',$data['image_path']);
	...
	{
	$stmt = $conn->prepare("UPDATE mmt_program SET tv_title=:tv_title, tv_description=:tv_description,status=:status, created_date=:created_date  WHERE id=:id");
	$stmt->bindParam(':tv_title',$data['tv_title']);
	$stmt->bindParam(':tv_description',$data['tv_description']);
	$stmt->bindParam(':status',$data['status']);
	$stmt->bindParam(':created_date',$data['created_date']);
...
function getAllNewsInNewsPage($conn)
{
